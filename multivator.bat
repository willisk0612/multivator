...
wt -M -p "Command Prompt" cmd /k "cd /d "%basePath%\lib\simulator" && .\SimElevatorServer.exe" ^
    ...
    ; focus-pane -t 0 ; split-pane -V -p "Command Prompt" cmd /k "cd /d "%basePath%\src" && go run main.go --id 0" ^
    ; focus-pane -t 1 ; split-pane -V -p "Command Prompt" cmd /k "cd /d "%basePath%\src" && go run main.go --id 1"